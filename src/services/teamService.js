...
async function createTeamPairing({
  ...
  round = Number(round);
  teamAId = validateId(teamAId, "team A ID");
  teamBId = validateId(teamBId, "team B ID");
...
if (teamAId === teamBId) {
  ...
  if (availableAt !== undefined && availableAt !== null) {
    availableDate = new Date(availableAt);

    if (Number.isNaN(availableDate.getTime())) {
      const error = new Error(
        "Invalid availableAt date."
      );
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const existingPairing =
      await tx.teamPairing.findFirst({
        where: {
          round,
          OR: [
            {
    ...
    if (existingPairing) {
      const error = new Error(
        "These teams have already been paired for this round."
      );
  ...
  try {
    const captainIds = [
      result.teamA.captainId,
      result.teamB.captainId,
    ].filter(Boolean);
      ...
      captainIds.map((captainId) =>
        notificationService.createNotification({
          ...
          message:
            ...
            `has been scheduled for Round ${result.round}.`,
        })
      )
    );
  } catch (error) {
    console.error(
      "TEAM PAIRING NOTIFICATION ERROR:",
      error
    );
  }
...
async function getTeamPairingById(pairingId) {
  pairingId = validateId(pairingId, "team pairing ID");

  const pairing = await prisma.teamPairing.findUnique({
    where: {
      id: pairingId,
    },
  ...
  if (!pairing) {
    ...
    error.code = "TEAM_PAIRING_NOT_FOUND";
...
async function getTeamPairings({
  ...
  const [pairings, total] =
    await prisma.$transaction([
      prisma.teamPairing.findMany({
        where,
        skip,
        take: limit,
